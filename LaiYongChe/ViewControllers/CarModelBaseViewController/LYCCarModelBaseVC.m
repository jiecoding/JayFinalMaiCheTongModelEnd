...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
        ...
        
    {
        ...
        for (int i = 0; i <operationCarCount; i++) {
            ...
            UIView *singleCellBgView = [[UIView alloc] initWithFrame:CGRectMake((_tableView.width/3 ) *i , 0, _tableView.width/3 -3 ,92)];
            singleCellBgView.backgroundColor = [UIColor whiteColor];
            [cell addSubview:singleCellBgView];
            
            UILabel *carNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(5, singleCellBgView.bottom - 30, singleCellBgView.width, 20)];
            carNameLabel.text = [NSString stringWithFormat:@"兰博基尼%d",i];
            carNameLabel.font = [UIFont getAdaptiveFontWithPix:13.0f];
            
            dic = [arr objectAtIndex:operationGetCarIndex -1];
            
            
            carNameLabel.text = [dic objectForKey:@"name"];
            
            [singleCellBgView addSubview:carNameLabel];
            
            
            UIImageView *carImageView = [[UIImageView alloc] initWithFrame:CGRectMake(singleCellBgView.width/2 -15, singleCellBgView.top +20, 30, 30)];
            ...
            if(i/2!=0){
                [carImageView sd_setImageWithURL:[NSURL URLWithString:urlStr] placeholderImage:[UIImage imageNamed:@"carLogo"]];
            }
            else
            {
                [carImageView sd_setImageWithURL:[NSURL URLWithString:urlStr2] placeholderImage:[UIImage imageNamed:@"carLogo"]];
                
            }
            [singleCellBgView addSubview:carImageView];
            
            UIButton *cellCollectionBtn = [UIButton buttonWithType:UIButtonTypeCustom];
            
//            [cellCollectionBtn setTitle:[NSString stringWithFormat:@"%d",operationGetCarIndex] forState:UIControlStateNormal];
            [cellCollectionBtn setTitle:carNameLabel.text forState:UIControlStateNormal];

            cellCollectionBtn.frame = CGRectMake(0, 0, singleCellBgView.width, singleCellBgView.height);
            
            [cellCollectionBtn addTarget:self action:@selector(cellCollectionBtn:) forControlEvents:UIControlEventTouchUpInside];
            
            [singleCellBgView addSubview:cellCollectionBtn];
...
- (void)cellCollectionBtn:(UIButton *)btn{
    
    DLog(@"[btn currentTitle]:%@",[btn currentTitle]);
}
